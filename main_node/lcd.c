...
unsigned char car_window[8]   = {0x1F, 0x11, 0x1B, 0x15, 0x1B, 0x11, 0x1F, 0x00};

unsigned char temp_symbol[8]  = {0x04, 0x0A, 0x0A, 0x0E, 0x0A, 0x1F, 0x0E, 0x00};

unsigned char man_symbol[8]   = {0x04, 0x0E, 0x0E, 0x04, 0x0E, 0x15, 0x04, 0x0A};

unsigned char engine_symbol[8]= {0x1F, 0x15, 0x1F, 0x11, 0x0A, 0x0A, 0x11, 0x00};
...
void buildCGRAM(u8 *p,u8 byte , u8 sym_code)

{

	u32 i;

	cmdLCD(0x40 + (sym_code*8));
...

{
    ...
    cmdLCD(0x40); for(i=0; i<8; i++) charLCD(up_arrow[i]);       // Char 0

    cmdLCD(0x48); for(i=0; i<8; i++) charLCD(down_arrow[i]);     // Char 1

    cmdLCD(0x50); for(i=0; i<8; i++) charLCD(car_window[i]);     // Char 2

    cmdLCD(0x58); for(i=0; i<8; i++) charLCD(temp_symbol[i]);    // Char 3

    cmdLCD(0x60); for(i=0; i<8; i++) charLCD(man_symbol[i]);     // Char 4

    cmdLCD(0x68); for(i=0; i<8; i++) charLCD(engine_symbol[i]);  // Char 5     // Char 6
